...
## [2026-06-18] ingest | Swap-panel redesign + executed-side arc (PRs #27–#35, post-Phase-11 GUI feedback; source docs/dev-log.md). One round per PR:
##   - PR #27 (FAR-leg overflow fix) — features/swaps.md: per-component bid/ask margin rows stack vertically inside each side so the FAR leg no longer overflows when tiles sit sm:flex-row.
##   - PR #28 (side-first redesign) — features/swaps.md + ticket.md: panel reworked to side-first per-direction tiles (swap-side-bid/ask, Buy/Sell vs Sell/Buy direction labels, data-quotable), replacing the SwapLegBlock + single net row.
##   - PR #29 (two-layer markup) — features/swaps.md: per-component leg markup re-added alongside all-in net.
##   - PR #30 (markup-mode toggle restored + NDF pair coercion) — features/swaps.md + ticket.md + scenario-player.md: mutually-exclusive Per-component/All-in toggle (swap-markup-mode); NDF coerced to a non-deliverable pair (NDF_PAIRS/isNdfPair, USDINR) in buildDeal.
##   - PR #31/#32 (per-side tiles, shared-spot + per-leg-fwd) — features/swaps.md: SwapSideTile; Per-component = one shared spot margin + per-leg forward-points margins (margin-input-{spot,near,far}-{bid,ask}); All-in = margin-input-net-{bid,ask}; effectiveSwapMargin folds optional spot (defaults 0); removed CLIENT BID/ASK tile labels; one-sided lock across both modes.
##   - PR #33 (markup-applied row + per-component breakdown) — features/historical-detail.md + data-models/deal-lifecycle-phase.md: SwapLegDetail gains a "Markup applied" row between street net (Net swap points) and Net used for execution; SwapMarkupDetail breakdown grid (spot/near/far/net) for PER_COMPONENT filtered by quoteSide; AppliedMargin swap variant gains optional components + quoteSide.
##   - PR #34 (executed side for two-way, FXSW-092) — data-models/deal.md + deal-event.md + components/scenario-player.md + deals-store.md: DealtSide (BID|ASK); CLIENT_ACCEPT carries executedSide; resolveExecutedSide (one-sided → only quotable side; two-way → seeded EXEC_SIDE_SEED flip; dealSides map set at inject, cleared in forgetDeal/reset); recordExecutedSide persists onto DealEntry/HistoricEntry (snapshot only when Executed); execution-side banner + dealt-column highlight / off-side dim.
##   - PR #35 (executed-side banner per-instrument labelling) — features/historical-detail.md: direction line only for single-leg instruments (spot/forward/NDF), suppressed for swaps; both Client Bid/Ask + Bank bid/ask shown for every instrument (clientSideLabelForDealtSide).
##   - NEW decisions/ADR-0017 — refines ADR-0014: side-first tiles + shared-spot/per-leg-forward markup model (over net-only / fully-independent per-leg alternatives); executed side captured + persisted + surfaced (off side dimmed, not dropped) for two-way requests; no new machine states.
##   - Also updated: index.md (refreshed swaps/historical-detail/scenario-player/deal/deal-event/deal-lifecycle-phase entries; registered ADR-0017), glossary.md (dealt/executed side, two-way request, all-in vs per-component markup, shared-spot markup).
##   - Code-drift: type/helper/data-* names checked verbatim vs SwapPanel.tsx, SwapSideTile.tsx, SwapLegDetail.tsx, HistoricDetailPanel.tsx, pips.ts, quoteSide.ts, player.ts, feed/types.ts, dealsStore.ts, dealsBootstrap.ts, lifecycle.ts. Canonical state names + data-* attributes unchanged.
##   - Brand-neutrality: case-insensitive vendor-name grep over wiki/ + raw/ → 0 hits (verified post-write). No phase summary existed for this arc; phase-11-swaps-summary.md (original build) NOT re-ingested.
