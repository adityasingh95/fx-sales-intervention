...
1. **No vendor names anywhere in `wiki/` or `raw/`.** Page bodies, headings, frontmatter, code blocks, link text, URLs, file names, alt text, comments inside YAML. The build-agent CLAUDE.md (`/CLAUDE.md`) permits vendor URLs in `/docs/` as research grounding because `/docs/` is not shipped; the wiki layer is held to a stricter rule — no vendor names, including in citation URLs. If a fact came from a vendor doc, paraphrase it from the relevant in-repo `/docs/` file and cite that file instead. The single source of vendor URLs in the repo remains `/CLAUDE.md` and `/docs/` (build-agent territory) — never the wiki.
...
10. **Sanity-check before each write.** Run a case-insensitive vendor-name grep on every page about to be written; if the count is non-zero, stop and revise. This is the lint version of rule §1 — cheaper to enforce per-write than per-lint-pass.
...
- **Lint**: `Lint` or `Lint {category}`. Standard checks + code-drift checks per schema. Also run a case-insensitive vendor-name grep over `wiki/` and `raw/` on every lint pass; any hit is a P0 finding.
