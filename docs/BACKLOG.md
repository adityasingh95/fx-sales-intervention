...
## Production-readiness track (future enhancement)

The prototype→production hardening track (resilience, observability, dev/prod
separation, security follow-through, accessibility, i18n, coverage, operability)
is parked under `docs/future-enhancements/` and is **not** part of the current
build scope:

- `docs/future-enhancements/production-readiness-spec.md` — the umbrella spec.
- `docs/future-enhancements/production-readiness-backlog.md` — the phased tickets.

Ticket and phase numbers there are deliberately placeholders (`FE-A-1`,
`FE-Phase-A`, …); real `FXSW-NNN` / `Phase-N` numbers are assigned from the
then-current high-water mark when each phase is scheduled, so ongoing side-fix
work can claim numbers freely in the meantime.
