...
## Production-readiness track (Phases 12–19)
...
Spec: `docs/11-production-readiness-spec.md` (sections referenced per ticket).
Every ticket inherits the standing cross-cutting gates (`docs/11` §10):
...
New ticket range: **FXSW-093 … FXSW-118**. (FXSW-092 was the executed-side work;
see `dev-log.md`.)

### Phase 12 — Resilience & error handling

The largest prototype→prod gap: no error boundaries, no global error handlers, and
unguarded state-machine dispatch. Specs: `docs/11` §1.

#### FXSW-093 — Error boundaries + recoverable fallback

**Effort:** M · **TDD:** Alongside · **Depends on:** — · **Docs:** `11` §1 (R1.1)
...
- New `src/components/ErrorBoundary.tsx` (class component; `getDerivedStateFromError`
  ...
  logging seam (FXSW-098) when present, else `console.error`.
...
#### FXSW-094 — Global error / unhandledrejection handlers

**Effort:** S · **TDD:** Alongside · **Depends on:** FXSW-093 · **Docs:** `11` §1 (R1.2)

**AC:**
- `src/main.tsx` registers `window.addEventListener('error', …)` and
  `'unhandledrejection'` handlers that route to the logging seam (FXSW-098 once
...
#### FXSW-095 — Guarded state-machine dispatch

**Effort:** M · **TDD:** Strict · **Depends on:** — · **Docs:** `11` §1 (R1.3)
...
#### FXSW-096 — Boundary input validation

**Effort:** M · **TDD:** Strict · **Depends on:** — · **Docs:** `11` §1 (R1.4)
...
#### FXSW-097 — Feed-disconnect / stale indicator + retry

**Effort:** M · **TDD:** Alongside · **Depends on:** — · **Docs:** `11` §1 (R1.5)
...
add consent-gated funnel analytics. Specs: `docs/11` §2.

#### FXSW-098 — Structured logging seam

**Effort:** S · **TDD:** Alongside · **Depends on:** — · **Docs:** `11` §2 (R2.1)

**AC:**
- New `src/lib/log.ts` exposes levelled logging (`debug/info/warn/error`) with no
  PII; a no-op sink in test; pluggable sink for FXSW-099.
- The lone `console.error` in `dealsStore.ts` and the handlers from FXSW-094 route
...
#### FXSW-099 — Error-tracking integration (env-gated)

**Effort:** M · **TDD:** Alongside · **Depends on:** FXSW-093, FXSW-098 · **Docs:** `11` §2 (R2.2)
...
- An industry-standard error-tracking client is initialised in `main.tsx` behind an
  ...
  boundary (FXSW-093) + global-handler (FXSW-094) errors.
- The provider origin is added to the runtime CSP `connect-src` (FXSW-105) — no
...
#### FXSW-100 — Consent-gated product analytics / RUM

**Effort:** M · **TDD:** Alongside · **Depends on:** FXSW-098 · **Docs:** `11` §2 (R2.3)
...
### Phase 14 — Dev/prod separation & bundle hygiene

Keep dev surfaces and test hooks out of the production artefact; add a bundle
budget. Specs: `docs/11` §3.

#### FXSW-101 — Gate + lazy-load DevInjector out of prod

**Effort:** M · **TDD:** Alongside · **Depends on:** — · **Docs:** `11` §3 (R3.1)
...
#### FXSW-102 — External-feed panel + test hooks inert in prod

**Effort:** S · **TDD:** Alongside · **Depends on:** FXSW-101 · **Docs:** `11` §3 (R3.2, R3.3)
...
#### FXSW-103 — CI bundle-size budget gate

**Effort:** S · **TDD:** n/a · **Depends on:** FXSW-101 · **Docs:** `11` §3 (R3.4)

**AC:**
- CI fails the build if the production bundle exceeds a documented budget; the
  budget is recorded in `docs/11` / CI config.

**Done when:** CI gate active; current build passes within budget.

### Phase 15 — Security follow-through & runtime hardening

Close the one deferred finding and operationalize for a real deployment. Specs:
`docs/11` §4.

#### FXSW-104 — Structural NDF spot-margin inertness

**Effort:** M · **TDD:** Strict · **Depends on:** — · **Docs:** `11` §4 (R4.1) · **Source:** `security/FXSW-081-review.md` F-2 (deferred from FXSW-089)
...
#### FXSW-105 — Runtime response headers + deploy hardening runbook

**Effort:** M · **TDD:** n/a · **Depends on:** — · **Docs:** `11` §4 (R4.2)
...
- The production deployment sets real response headers (CSP, HSTS,
  ...
  origins (FXSW-099) with no wildcard.
...
#### FXSW-106 — CI security gates

**Effort:** S · **TDD:** n/a · **Depends on:** — · **Docs:** `11` §4 (R4.3)
...
Depth items for a customer-facing AA bar. Specs: `docs/11` §5.

#### FXSW-107 — Modal focus management

**Effort:** M · **TDD:** Alongside · **Depends on:** — · **Docs:** `11` §5 (R5.1)
...
#### FXSW-108 — Blotter labels + live-region announcements

**Effort:** M · **TDD:** Alongside · **Depends on:** — · **Docs:** `11` §5 (R5.2)
...
#### FXSW-109 — Keyboard/tab-order + automated a11y gate

**Effort:** M · **TDD:** Alongside · **Depends on:** FXSW-107, FXSW-108 · **Docs:** `11` §5 (R5.3, R5.4)
...
### Phase 17 — Internationalization & configuration

Localizable strings + externalized constants for a customer-facing build. Specs:
`docs/11` §6.

#### FXSW-110 — i18n infrastructure + catalog extraction

**Effort:** L · **TDD:** Alongside · **Depends on:** — · **Docs:** `11` §6 (R6.1)
...
#### FXSW-111 — Locale-aware formatting

**Effort:** M · **TDD:** Alongside · **Depends on:** FXSW-110 · **Docs:** `11` §6 (R6.2)
...
#### FXSW-112 — Externalize thresholds + timings into config

**Effort:** S · **TDD:** Alongside · **Depends on:** — · **Docs:** `11` §6 (R6.3)
...
### Phase 18 — Testing & quality gates

Coverage visibility, edge-case/error-path depth, cross-browser. Specs: `docs/11` §7.

#### FXSW-113 — Coverage reporting + thresholds

**Effort:** S · **TDD:** n/a · **Depends on:** — · **Docs:** `11` §7 (R7.1)
...
#### FXSW-114 — Edge-case + error-path suites

**Effort:** M · **TDD:** Strict · **Depends on:** FXSW-093, FXSW-096, FXSW-104 · **Docs:** `11` §7 (R7.2)
...
#### FXSW-115 — Cross-browser E2E

**Effort:** M · **TDD:** Acceptance · **Depends on:** — · **Docs:** `11` §7 (R7.3)
...
### Phase 19 — Performance, scalability & operations

Bound growth, smooth fan-out, and make it deployable/operable. Specs: `docs/11` §8.

#### FXSW-116 — Historic-store bound

**Effort:** S · **TDD:** Alongside · **Depends on:** — · **Docs:** `11` §8 (R8.1)
...
#### FXSW-117 — Feed render perf + poller parallelization

**Effort:** M · **TDD:** Alongside · **Depends on:** — · **Docs:** `11` §8 (R8.2)
...
#### FXSW-118 — Deployment & ops

**Effort:** M · **TDD:** n/a · **Depends on:** FXSW-105 · **Docs:** `11` §8 (R8.3)
