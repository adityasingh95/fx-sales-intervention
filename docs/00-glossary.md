...
Domain terms used across this prototype. When in doubt, prefer the Caplin term over a generic synonym.

## Caplin / Sales Intervention terms
...
| **Ticket (SI ticket)** | The pricing panel a sales trader opens for a single deal. Six product variants in the full product; **only Spot in this prototype**. |
...
## Caplin state names (RFS Trade Model)

States from the RFS trade model used in this prototype. Full set documented at https://docs.caplin.com/developer/fx-sales/st-implementing-sales-intervention.
...
## Caplin state names (Sales Intervention Trade Model)
...
The real product publishes these in a container at `/PRIVATE/FX/SALES/BLOTTER/ACTIVEDEALS`. This prototype keeps the field names on the `Deal` type for fidelity.

## Internal terms (this prototype only)

| Term | Definition |
|---|---|
| **Dummy feed** | The in-memory pricing + deal simulator. See `04-dummy-feed-spec.md`. |
| **Scenario** | A pre-canned sequence of events used for demos and E2E tests. See `07-scenario-pack.md`. |
| **Dev Injector** | The hidden control panel at `?dev=1` that lets the operator inject scenarios on demand. |
